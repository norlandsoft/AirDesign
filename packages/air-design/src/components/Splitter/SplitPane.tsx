 ...
 * SplitPane 可拖拽分割面板
 *
 * 保留旧版完整拖拽/折叠逻辑（mousedown/mousemove/mouseup + 触摸事件、min/max/step、collapsible
 * 折叠按钮、primary first/second），仅替换依赖：移除 react-lifecycles-compat polyfill（React 16+ 已原生
 * 支持 getDerivedStateFromProps）、移除 svgr ?react 图标改用内联 SVG、移除 .less 样式改用内联/Tailwind。
 *
...
class SplitPane extends React.Component<SplitPaneProps, SplitPaneState> {
  ...
  constructor(props: SplitPaneProps) {
    ...
    const initialSize = size !== undefined ? size : getDefaultSize(defaultSize, minSize, maxSize, undefined)
  ...
  componentDidMount() {
    ...
    this.setState((prevState) => ({...prevState, ...SplitPane.getSizeUpdate(this.props, this.state)}))
  }
  ...
  onMouseDown = (event: React.MouseEvent) => {
    ...
    this.onTouchStart({touches: [touch], changedTouches: [touch], targetTouches: [touch]} as unknown as React.TouchEvent)
  }
  ...
  private onMouseMove = (event: MouseEvent) => {
    const eventWithTouches = {touches: [{clientX: event.clientX, clientY: event.clientY}]}
    ...
    if (allowResize && active && position !== undefined) {
      ...
      if (ref && ref2) {
        ...
        if (node.getBoundingClientRect && node2) {
          ...
          const current = split === 'vertical' ? (event as TouchEvent).touches[0].clientX : (event as TouchEvent).touches[0].clientY
          ...
          if (maxSize !== undefined && typeof maxSize === 'number' && maxSize <= 0) {
            const splitPaneEl = this.splitPane
            if (splitPaneEl) {
              newMaxSize = split === 'vertical' ? splitPaneEl.getBoundingClientRect().width + maxSize : splitPaneEl.getBoundingClientRect().height + maxSize
            }
          }
          ...
          const newMaxSizeAsNumber = typeof newMaxSize === 'string' ? parseFloat(newMaxSize) : (newMaxSize ?? Infinity)
          ...
          onChange?.(newSize)
          onResize?.(newSize, this.state.collapsed)

          const newSizeAsNumber = typeof newSize === 'string' ? parseFloat(newSize) : newSize
          const newState = {draggedSize: newSizeAsNumber, [isPrimaryFirst ? 'pane1Size' : 'pane2Size']: newSizeAsNumber} as const
  ...
  render() {
    ...
    if (split === 'vertical') {
      ...
      Object.assign(style, {flexDirection: 'column', minHeight: '100%', width: '100%'})
    }

    const classes = ['SplitPane', className, split, disabledClass].filter(Boolean)
    const pane1Style = {...paneStyle, ...pane1StyleProps}
    const pane2Style = {...paneStyle, ...pane2StyleProps}
    const pane1Classes = ['Pane1', paneClassName, pane1ClassName].filter(Boolean).join(' ')
    const pane2Classes = ['Pane2', paneClassName, pane2ClassName].filter(Boolean).join(' ')

    return (
      <div className={classes.join(' ')} ref={(node) => {this.splitPane = node}} style={style}>
        <Pane className={pane1Classes} key="pane1" eleRef={(node) => {this.pane1 = node}} size={collapsed ? 0 : pane1Size} split={split} style={pane1Style}>
        ...
        <Pane className={pane2Classes} key="pane2" eleRef={(node) => {this.pane2 = node}} size={pane2Size} split={split} style={pane2Style}>
          {notNullChildren[1]}
          {collapsible && split === 'vertical' && (
            <div
              className="absolute flex cursor-pointer items-center text-muted-foreground hover:text-foreground"
              style={{bottom: `${togglePosition}px`, left: collapsed ? '-5px' : '-7px'}}
              onClick={() => {
