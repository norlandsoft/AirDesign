...
const AirTree: React.FC<TreeProps> = props => {
  ...
  const renderMenuBar = (data, menu) => {
    return menu ? (
        <Dropdown.Menu>
          {
            menu.map(item => {
              return item.type === 'divider' ? <Dropdown.Divider key={randomString(12)}/> :
  ...
  const renderLabel = (label, data) => {
    ...
    const hasMenu = (data.type === 'group' && (data.menu || groupMenu)) || (data.type === 'item' && (data.menu || itemMenu) || (data.type !== 'group' && data.type !== 'item' && itemMenu));
    ...
    return (
        ...
        >
          <div className={'air-tree-node-icon'}>
            {
              data.image ? <Icon name={data.image} size={18}/> :
