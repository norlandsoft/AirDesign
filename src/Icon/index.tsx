...
const Icon: React.FC<IconsProps> = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!raw) {
        raw = iconData[name.toLowerCase()] ?? iconData[name.toUpperCase()] ?? iconData[name.charAt(0).toUpperCase() + name.slice(1).toLowerCase()];
      }
      if (!raw) {
        setError(name);
        setLoading(false);
      ...
      if (svgElement) {
        ...
        const allElements = svgElement.querySelectorAll('g, path, circle, rect, line, polyline, polygon');
  ...
  if (error) {
    return <div className={className} style={{ width: size, height: size, color: 'red' }}>{error}</div>;
  }
  return (
    <div
      className={className}
