...
// Mock 支持：当 mock 模式启用时，SSE_POST 使用 mock 实现
declare const __MOCK_ENABLED__: boolean;
declare function mockSSEPost(url: string, params: any, callback: (data: string) => void, idleTimeout?: number): void;
...
export async function SSE_POST(
  ...
  // Mock 模式：使用 mock SSE 实现
  if ((window as any).__MOCK_ENABLED__) {
    const {mockSSEPost} = await import('@/mock/mockInterceptor');
    return mockSSEPost(String(url), params, callback, idleTimeout);
  }
