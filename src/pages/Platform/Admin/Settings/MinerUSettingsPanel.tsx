 ...
 * MinerU 设置面板
 *
 * 用于配置 MinerU 文档提取服务的地址和端口。
 * 配置持久化到网关 EmbeddedStorage（H2），通过 /admin/paas/mineru 接口读写。
 *
 * Created by ChaiMingXu, on 2026/05/12
 */
const MinerUSettingsPanel: React.FC<any> = (props) => {
  ...
  useEffect(() => {
    setLoading(true);
    dispatch({
      type: 'platform/fetchMinerUConfig',
      callback: (resp: any) => {
        setLoading(false);
  ...
  const onFinish = (values: any) => {
    setSaving(true);
    dispatch({
      type: 'platform/saveMinerUConfig',
      payload: {
        host: values.host,
        port: values.port,
      },
  ...
  return (
      <div className="air-grid-panel">
        <div className="air-grid-panel-top">
          <div className="air-grid-panel-title">MinerU</div>
        ...
        <div className="admin-paas-panel-content" style={{height: contentHeight, overflow: 'auto'}}>
          ...
          >
            ...
            <Form.Item label="端口" name="port" rules={[{required: true, message: '请输入端口'}]}>
              <InputNumber min={1} max={65535} placeholder="默认 9103" style={{width: '100%'}}/>
            </Form.Item>
          </Form>
        </div>
      </div>
