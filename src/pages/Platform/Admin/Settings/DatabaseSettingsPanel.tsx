 ...
 * 数据库设置面板
 *
 * 布局与用户设置、角色设置一致：air-grid-panel（标题栏 + 内容区）。
 * 用于配置平台主数据库连接（PostgreSQL）。
 * 配置持久化到网关 EmbeddedStorage（H2），通过 /admin/paas/database 接口读写。
 *
...
const DatabaseSettingsPanel: React.FC<any> = (props) => {
  ...
  useEffect(() => {
    setLoading(true);
    dispatch({
      type: 'platform/fetchDatabaseConfig',
      callback: (resp: any) => {
        setLoading(false);
        if (resp?.success && resp?.data) {
          ...
          form.setFieldsValue({
            ...
            port: d.port ?? 5432,
            database: d.database ?? 'air_platform',
  ...
  const onFinish = (values: any) => {
    setSaving(true);
    dispatch({
      type: 'platform/saveDatabaseConfig',
      payload: {
        driver: 'postgresql',
        host: values.host,
        port: values.port,
        ...
        <div className="admin-paas-panel-content" style={{height: contentHeight, overflow: 'auto'}}>
          ...
          >
            ...
            <Form.Item label="数据库名" name="database" rules={[{required: true, message: '请输入数据库名'}]}>
              <Input placeholder="如 air_platform"/>
            </Form.Item>
