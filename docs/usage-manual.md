...
`type`：`small`(290) / `default`(378) / `large`(850) / `huge`(1280) / `full` / `custom`(用 width)。Header 高度 40px，Footer 高度 50px（按钮间距 8px；非 full 靠左，full 靠右），打开时自侧边滑入。支持内嵌抽屉（`innerDrawer`/`showInnerDrawer`/`onInnerClose`）。
