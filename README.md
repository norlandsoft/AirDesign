...
| **air-kit** | `packages/air-kit` | 业务前端脚手架（登录页、安全布局、用户设置、跨应用切换、HttpRequest 等），仅服务于自有服务 |

架构设计详见 [docs/architecture.md](docs/architecture.md)，接入与组件用法详见 [docs/usage-manual.md](docs/usage-manual.md)。各包 API 细节见 `packages/air-design/README.md` 与 `packages/air-kit/README.md`。
...
npm workspaces 会将 `packages/air-design` 与 `packages/air-kit` 的依赖统一安装到根目录 `node_modules/`，子包之间通过 workspace 协议自动链接。
...
npm run build -w air-kit      # 构建业务 SDK（依赖 air-design 已构建）
```
...
# 仅构建 air-kit（需 air-design 已构建，或在 workspace 中已 link）
npm run build:sdk
```
...
修改 `packages/air-kit/src/` 后自动重新构建到 `packages/air-kit/dist/`。
...
**air-kit**（`packages/air-kit/dist/`）：

```
dist/
├── index.mjs              # 主入口
├── air-kit.css            # 业务组件样式（必须引入）
├── src/index.d.ts         # TypeScript 类型入口
├── config.mjs             # 配置模块
...
cd /path/to/your-app
npm install air-design air-kit
```
...
npm run build
# 产物位于 packages/air-design/dist 与 packages/air-kit/dist
```
...
```bash
cd /path/to/your-app
npm unlink air-design air-kit

cd /opt/AirDesign
npm unlink -w air-design
npm unlink -w air-kit
```
...
```json
{
  "dependencies": {
    ...
    "air-kit": "git+ssh://git@your-host/AirDesign.git#master:packages/air-kit"
  }
}
```
...
### 使用 air-design + air-kit（企业应用标准方案）
...
export { UserModel } from 'air-kit';

// UmiJS 约定：将 UserModel 放入 src/models/user.ts
// 内容可直接 re-export：
// export { UserModel as default } from 'air-kit';
```
...
```typescript
import { defineSdkConfig } from 'air-kit/config';
import UserModel from 'air-kit/models/user';
import SecurityLayout from 'air-kit/layouts/SecurityLayout';
import Login from 'air-kit/pages/Login';
import { POST } from 'air-kit/utils/HttpRequest';
```
...
- [ ] `package.json` 添加 `air-design`、`air-kit` 依赖
...
原 `/opt/AirFramework/frontend-sdk`（包名 `air-auth`）已合并为 `air-kit`：
...
```tsx
import 'air-design/style.css'
import 'air-kit/style.css'   // 使用 SDK 时
```
...
- [air-kit 业务模块说明](packages/air-kit/README.md)
